...
module.exports = function lifejacket(sails) {
  ...
  return {
    ...
    configure: function (){
      ...
      if (sails.config.lifejacket.acmeChallengeToken !== undefined && !_.isString(sails.config.lifejacket.acmeChallengeToken)) {
        throw new Error('Configuration error in LetsEncrypt hook: If set, "lifejacket.acmeChallengeToken" must be set as a valid string.  Instead, got: '+util.inspect(sails.config.lifejacket.acmeChallengeToken));
      }
      if (sails.config.lifejacket.acmeKeyAuthorization !== undefined && !_.isString(sails.config.lifejacket.acmeKeyAuthorization)) {
        throw new Error('Configuration error in LetsEncrypt hook: If set, "lifejacket.acmeKeyAuthorization" must be set as a valid string.  Instead, got: '+util.inspect(sails.config.lifejacket.acmeKeyAuthorization));
      }

      // If `host` config was provided, or if falling back to sails.config.custom.baseUrl yields something...
      sails.config.lifejacket.host = sails.config.lifejacket.host || (sails.config.custom&&sails.config.custom.baseUrl) || undefined;
      ...
      before: {
        ...
        '/.well-known/acme-challenge/*': {
          ...
          fn: function (req, res, next) {
            ...
            if (!sails.config.lifejacket.acmeChallengeToken || !sails.config.lifejacket.acmeKeyAuthorization) {
              return next();
            }//•

            // Only respond if the supposed challenge token is valid.
            var supposedChallengeToken = req.param('0');
            var actualChallengeToken = sails.config.lifejacket.acmeChallengeToken;
            if (supposedChallengeToken !== actualChallengeToken) {
              return next();
            }//•

            var keyAuthorization = sails.config.lifejacket.acmeKeyAuthorization;
